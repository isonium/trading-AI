...
Portfolio::~Portfolio(){
	for(auto position_pair: positions){
		...
		for(Position * position: *position_vector) delete position;
		delete position_vector;
	}
}

const std::vector<Position*> * Portfolio::find_position(const std::string ticker) const{
...
void Portfolio::remove_stocks(const std::string ticker, long quantity){
	std::vector<Position*> * positions_vector = const_cast<std::vector<Position*> *>(find_position(ticker));
	long total_owned = get_ticker_quantity(ticker);
	if(total_owned < quantity) throw "This portfolio does not have enough stocks with this ticker";
